...
//
// D-12: the 3x-poll-interval factor (STALENESS_FACTOR, config.ts) is deliberately wider
...
import type { DevicePayload, PollerStatusPayload } from "./types";
...
export function isPollerStale(
  pollerStatusPayload: PollerStatusPayload | null | undefined,
  nowMs: number = Date.now(),
): boolean {
...
export function pollerOfflineSince(
  pollerStatusPayload: PollerStatusPayload | null | undefined,
  ...
  //
  // `fallbackRaw` (useAppStore's lastKnownPollerTimestamp) covers a third case: the
