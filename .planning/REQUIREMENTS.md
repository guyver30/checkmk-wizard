...
- [ ] **PLR-01**: Poller connects to Checkmk Livestatus over TCP on a configurable interval and queries current host state, service-state summary counts, tags, and parent/folder info
...
- [ ] **PLR-03**: Poller publishes per-device retained status to `lan/devices/{id}/status` every cycle, preserving Checkmk's OK/WARN/CRIT/UNKNOWN/DOWN granularity (not collapsed to plain up/down), including a timestamp for staleness detection
...
- [ ] **PLR-08**: Poller surfaces Checkmk downtime/acknowledgement state in the per-device status payload
