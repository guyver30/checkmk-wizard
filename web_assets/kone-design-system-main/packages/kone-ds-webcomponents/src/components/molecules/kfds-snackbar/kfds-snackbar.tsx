...
@Component({
  tag: 'kfds-snackbar',
  styleUrl: 'kfds-snackbar.scss',
  shadow: true,
})
export class KfdsSnackbar {
  @Prop() type: string = SnackbarType.Info;
  @Prop() verticalPosition: string = SnackbarVerticalPosition.Bottom;
  @Prop() horizontalPosition: string = SnackbarHorizontalPosition.Left;
  ...
  render() {
    ...
    return (
      <Host>
        {this.visible ?
          <div part='snackbar-container' id={this.elementId} class={this.type + ' kfds-snackbar-container ' + this.customClass + ' ' + this.verticalPosition + ' ' + this.horizontalPosition}>
            ...
            {this.showClose ? <a part='close' class='close' onClick={() => this.removeSnackbar()}><img src={CloseCrossIcon} /></a> : null}
          </div>
        : null }
      </Host> 
    );
