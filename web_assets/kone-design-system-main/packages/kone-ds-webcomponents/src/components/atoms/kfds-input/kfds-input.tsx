...
export class KfdsInput {
  ...
  @Prop() supportingText = '';
  @Prop() disabled = false;
  @Prop() size: InputSize = InputSize.Small;
  @Prop() shape: InputShape = InputShape.Rounded;
  @Prop() maxLength = -1;
  ...
  render() {
    return (
      <Host>
        <div part='input-container' id={this.elementId} class={`${this.size} kfds-input-container ${this.customClass} ${this.floating ? 'floating' : ''}`}>
          <label part='input-label' class={'kfds-input-label'}>
          ...
          {this.supportingText || this.maxLength > 0 ? (
            <div part='input-info-container' class={'kfds-input-info-container'}>
              {this.supportingText && (
                <span part='input-supporting-text' class={'kfds-input-supporting-text'}>
                  {!this.floating && this.getSupportingTextIcon()}
                  {this.supportingText}
                </span>
              )}
              {this.maxLength > 0 && (
                <span part='input-char-length' class={'kfds-input-char-length'}>
